...
data class CurrencyInfo(
    ...
    val flag: String,
    /**
     * Сколько знаков после запятой получает НОВАЯ группа в этой валюте. Шкала —
     * свойство группы, а не валюты: здесь только умолчание.
     */
    val displayExponent: Int = 0,
    /**
     ...
     */
    val maxExponent: Int = 0,
    /**
     * Разрешено ли сейчас ВВОДИТЬ дроби. Признак сервера, одинаковый во всех
     * строках справочника. Старый сервер его не шлёт, и отсутствие читается как
     * запрет: разрешение должно приходить явно.
     */
    val fractionalInput: Boolean = false,
)
...
data class RoomSummary(
    ...
    val currency: String,
    /**
     * Шкала ЭТОЙ группы: сколько знаков после запятой у её сумм. Ноль — суммы
     * целые. Старый сервер поля не шлёт, и ноль тут единственное честное
     * значение: выводить шкалу из справочника валют нельзя, суммы в ответе
     * всё равно целые.
     */
    val displayExponent: Int = 0,
    /**
     * Растёт при каждой смене шкалы группы. По нему офлайн-очередь узнаёт, что
     * её снимок устарел: путь 0 → 2 → 0 вернул бы прежнюю шкалу, а суммы за это
     * время пересчитали дважды.
     */
    val scaleVersion: Int = 0,
    ...
    /**
     * Непрочитанные события ЭТОЙ группы: бейдж на вкладке сообщает, ЧТО что-то
     * случилось, счётчик на карточке — ГДЕ. Гаснет открытием самой группы.
     * `100` означает «больше 99» (см. badgeLabel в MainScaffold).
     *
...
data class RoomDetail(
     ...
     *
     * null — ответ старого сервера или комната из офлайн-кеша прошлой версии:
