...
class MainActivity : ComponentActivity() {
    ...
    override fun onStart() {
        ...
        // Тем же поводом досылаем накопленные события: после перезапуска на
        // диске лежит хвост прошлого сеанса.
        analytics.onForegrounded()
    ...
    private fun handleDeepLink(intent: Intent?) {
        ...
        // Синхронно и ДО записи на диск: приветствие теперь стоит перед входом,
        // и первый кадр холодного старта по ссылке рисуется раньше, чем
        // DataStore успевает ответить. Без этой отметки приглашённый увидел бы
        // рассказ о продукте вместо тусы, в которую его позвали.
        pendingJoinStore.noteArrived()
        appScope.launch {
