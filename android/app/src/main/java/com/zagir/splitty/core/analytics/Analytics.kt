 ...
 *
 * Всё, что до входа, идёт обезличенным маршрутом ([trackAnonymous]): холодный
 * app_open, экран входа и — с тех пор, как приветствие переехало вперёд, —
 * весь онбординг.
 */
@Singleton
class Analytics @Inject constructor(
    ...
    fun trackSignedIn(event: AnalyticsEvent, userId: Long, token: String) {
        ...
        // Двойник входа — в ПРЕЖНЕЙ, обезличенной сессии, и потому строго до
        // ротации. Здесь, а не в трёх методах входа по отдельности: они все
        // сходятся сюда, и порядок перестаёт зависеть от вызывающего.
        trackAnonymous(AnalyticsEvent.AuthCompleted)
        rotateSessionFor(userId)
