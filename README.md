...
«Уведомления». Позвать **можно только друга**; постороннему по-прежнему отправляют ссылку
(`inviteUrl` / диплинк ниже), и он входит по ней сам. Placeholder-участников и приглашений
по email в Splitty нет.

Ушедшего (`left`) или отказавшегося (`declined`) молча вернуть нельзя: повторное приглашение
создаёт `pending` и ждёт его явного согласия.

**Выход из группы запрещён, пока на человеке висят операции** — он донор или получатель
с долей хотя бы одного активного расхода. Иначе выход порвал бы расчёты остальных: доли
