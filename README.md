...
> `migration`; фильтр узкий, так что и без маркера повтор безвреден; аккаунты из `POST /auth/dev`
> отсекаются по собственной метке `dev_auth`, а не по режиму инсталляции). **Изменилось не значение,
> а смысл.** Новые
...
> ⚠️ **Метка `dev_auth` защищает только те dev-аккаунты, что созданы этой версией и позже.**
> Пишет её `UpsertUser` в момент входа через `POST /auth/dev`, а бэкфилл выполняется на старте —
> так что на dev/staging-инсталляции, где `/auth/dev`-аккаунты завели ДО обновления, метки на них
> нет, и первый же старт новой версии проставит им `telegram_id = _id`. Продакшена это не
> касается: без `API_DEV_AUTH` маршрут отдаёт 404, а в `docker-compose` флага нет. Перед
> обновлением dev-инсталляции нужно либо проставить `dev_auth: true` существующим тестовым
> аккаунтам, либо вписать маркер `backfill_telegram_id` в коллекцию `migration`, либо просто
> снести dev-базу.
