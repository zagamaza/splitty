...
//     с обеих сторон.
//
//   - product_events — содержимого нет, но есть user_id: когда человек открывал
...
func (s *Server) purgeUserData(ctx context.Context, u *api.User) error {
	...
	for _, cleaner := range []struct {
		...
		// Продуктовые события: содержимого в них нет, но есть user_id, то есть
		// поведение конкретного человека — когда открывал приложение, что
		// делал, где оборвался. Подключается БЕЗУСЛОВНО, независимо от того,
		// включён ли приём: иначе выключенная аналитика давала бы аккаунт,
