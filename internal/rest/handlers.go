...
func (s *Server) handleUpdateOperation(w http.ResponseWriter, r *http.Request) {
	...
	// человек на другом телефоне увидел бы, что сумма изменилась сама.
	// Отвергаем: пусть обновится и пришлёт точную величину.
	if req.SumMinor == nil && oldOp.SumMinor != nil && *oldOp.SumMinor%api.MinorFactor != 0 {
		writeError(w, http.StatusConflict, "conflict",
			"расход записан с копейками — обновите приложение, чтобы его править")
		return
	}
