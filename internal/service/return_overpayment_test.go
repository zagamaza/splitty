...
func TestGetRoomDebtsByKazahRoom(t *testing.T) {
	...
	// Комната рублёвая: долги кратны рублю, и остаток меньше шага долгом не
	// становится — прежний артефактный «долг» в 1 ₽ у No Mercy исчез вместе
	// с ним.
	...
	for _, d := range debts {
		total += d.Sum
		assert.Zero(t, d.SumMinor%100, "долг не кратен рублю — его не погасить")
	}
	assert.Equal(t, 10925, total, "сумма рёбер равна сумме положительных позиций")
	assert.Len(t, debts, 3, "лишних транзитных рёбер быть не должно")
}
