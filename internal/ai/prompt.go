...
- Отвечай ТОЛЬКО валидным JSON по схеме, без пояснений.`)
	...
	// Язык ОТВЕТА, а не распознавания: questions читает человек. Без этого
	// правила модель шла за языком промпта и спрашивала по-русски у японца.
	// Описания позиций не переводим — человек продиктовал их сам.
	if in.Lang != "" {
		b.WriteString(fmt.Sprintf(
			"\n- Тексты в поле questions пиши на языке %s. Названия позиций и имена оставляй как есть.",
			in.Lang))
	}
