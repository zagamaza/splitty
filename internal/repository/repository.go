...
//
// Глобальной уникальности тут нет и не обещается: тот же токен может остаться
// у прежнего аккаунта, если logout не дошёл до сервера. Это отдельная история
// (чужие пуши на переданное устройство), и решается она отбраковкой токена по
// ответу FCM, а не этим методом.
//
