...
//
// Глобальной уникальности тут нет и не обещается: тот же токен остаётся у
// прежнего аккаунта, если logout не дошёл до сервера, и регистрация на новый
// аккаунт его оттуда НЕ убирает.
//
// Само это не рассосётся. Токен FCM принадлежит установке приложения, а не
// сессии: после смены аккаунта он остаётся валидным, доставка идёт успешно, и
// отбраковка по UNREGISTERED (см. push.Worker) не сработает — токен не мёртв.
// То есть прежний аккаунт продолжит слать названия групп и расходов на
// устройство, которым пользуется уже другой человек. Отдельная задача, и это
// privacy, а не гигиена.
//
