...
func (tfd TelegramFileRecipient) GetFile(fileID string) (io.ReadCloser, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "get file direct URL (fileID: %service)", fileID)
	}

	resp, err := tfd.httpClient.Get(url)
	if err != nil {
		return nil, errors.Wrapf(err, "file by direct URL (fileID: %service)", fileID)
		// don't expose `url` – it contains Bot API Token
	}

	if resp.StatusCode != 200 {
		return nil, errors.Wrapf(err, "non-200 response from file direct URL (fileID: %service)", fileID)
	}
