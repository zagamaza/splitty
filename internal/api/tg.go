...
type User struct {
	...
	// CreatedAt — когда аккаунт заведён. Ставится СЕРВЕРОМ при вставке и
	// больше не меняется.
	//
	// Клиент отличить регистрацию от входа не может: respondWithToken отвечает
	// одинаково и новому, и существующему, поэтому событие «зарегистрировался»
	// с телефона было бы враньём для telegram, google и apple. Указатель, а не
	// значение: у 267 аккаунтов, заведённых до этой правки, даты нет, и они
	// отдельная когорта «до аналитики», а не 1 января 1970-го.
...
func (u User) Snapshot() User {
	...
	// Дата регистрации — состояние аккаунта, а не личность участника. В снимке
	// внутри комнаты она была бы копией, переживающей анонимизацию, и при этом
	// ни долгам, ни расчёту не нужна
	u.CreatedAt = nil
	return u
}
