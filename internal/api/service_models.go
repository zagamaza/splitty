...
type Room struct {
	...
	// FractionalAmounts — считает ли туса копейки. Указатель: у тус, заведённых
	// до появления настройки, поля нет, и тогда берётся умолчание валюты.
	//
	// ⚠️ Хранение от этого признака НЕ зависит: деньги всегда лежат в копейках.
	// Признак решает, что принимать на вводе и с какой точностью делить расход,
	// поэтому его переключение не трогает ни одной записи.
	FractionalAmounts *bool `json:"fractionalAmounts,omitempty" bson:"fractional_amounts,omitempty"`
}
...
type CurrencyInfo struct {
	...
	// FractionalDefault — считает ли НОВАЯ туса в этой валюте копейки.
	// У рубля ложь, хотя копейки существуют: расхождение с ISO 4217 осознанное.
	FractionalDefault bool
	// SupportsFraction — есть ли у валюты дробная часть в обороте. Ложь у иены
	// и воны: переключателя копеек в такой тусе нет вовсе.
