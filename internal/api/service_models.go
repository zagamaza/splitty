...
type Room struct {
	...
	// Revision растёт при ЛЮБОЙ записи в документ комнаты. По ней пересчёт
	// шкалы понимает, что комнату трогали между чтением и записью.
	//
	// ⚠️ Версии операций для этого не годятся: отметка о доставке пуша,
	// архивирование расхода и затирание личности в снимках участников
	// version не поднимают вовсе, а пересчёт кладёт массив операций целиком —
	// и вернул бы удалённый расход, второй пуш и стёртые данные человека.
	Revision int `json:"-" bson:"revision,omitempty"`
}
