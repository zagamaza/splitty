...
func TestFillMoneyOnLegacyDocument(t *testing.T) {
	...
	// ⚠️ Доли 6,67 + 6,67 + 6,66 в копейках дают ровно итог, поэтому вектор
	// ЗАПИСАН и переносится как есть. Раньше он выбрасывался и выводился заново
	// целым шагом (700/700/600), а погашения в старых тусах делались против
	// прежних долей — на сверке прода это воскрешало долги в компаниях, которые
	// уже рассчитались. Выводим заново только когда записанного вектора нет:
	// доли вроде 33,33 в итог не сходятся и сюда не попадают.
