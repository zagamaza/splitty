...
Отчёт: `docs/plans/20260719-android-acceptance.md`.

- [x] Таблица паритета по трём gap-отчётам: каждый пункт «Отсутствует» закрыт или осознанно отложен (с пометкой) — все три отчёта закрыты, 27/27 пунктов дизайн-ревью подтверждены кодом
- [x] Инварианты: itemized iOS↔Android проверен полной петлёй (`ItemizedCrossClientTest`), process death — структурно (черновик/аудио/фото в SavedStateHandle + round-trip тест). **Отложено на железо:** замер латентности мика и `am kill`. По ходу найден и починен реальный баг: пороги жеста сравнивались в пикселях с dp-порогами (на 3x замок срабатывал через ~23 dp вместо 70)
- [x] Дизайн-ревью паритета: три независимых прохода выполнены, токены 1:1, блокеры исправлены (a11y диаграмм, хептики, parsing-оверлей, копия). **Отложено:** сверка с iOS-снапшотами бок-о-бок (эталоны iOS не в репо, нужен xcodebuild) и тюнинг «ощущения» глазами — список косметики в отчёте
- [x] Матрица: API 26 покрыт статически (lint без `NewApi`), обе темы — снапшотами, семантика TalkBack приведена в порядок. **Прогон на эмуляторах/железе не выполнен** (устройство не подключено) — единственный реально открытый пункт, перенесён в Post-Completion
- [x] `testDebugUnitTest + verifyRoborazziDebug + lint` + R8-smoke зелёные — 306 тестов, 23 снапшота (воспроизводимость починена: волна пинится `frozenNowMs`)
...
- **Прогон матрицы устройств (из Task 15, не выполнен без железа):** API 26 / 33
  (permissions, транскрайбер) / 35–36 (edge-to-edge, predictive back), слабый
  OEM, TalkBack smoke, 200% шрифт; замер латентности мика (<50 мс) и process
  death через `am kill`; сверка Roborazzi с iOS-снапшотами бок-о-бок
  (`xcodebuild` из `ios/`). Детали — `20260719-android-acceptance.md`.
