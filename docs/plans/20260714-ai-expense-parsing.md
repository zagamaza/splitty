...
- [ ] **загрузка `items` при правке (критично — иначе правка itemized-операции стирает позиции):** в `AddExpenseViewModel.load(editOperation:)` (`AddExpenseViewModel.swift:179`) сейчас читаются только `recipients`/`splitType`/`amountTexts`. Добавить чтение `editOperation.items` в форму, чтобы при редактировании чек не превращался в плоскую операцию (иначе PUT уйдёт без items → сервер затрёт Items в Task 8). ⏸ ОТЛОЖЕНО В TASK 13: требует draft-представления формы (композер/шит позиций/сброс Items), которого пока нет — половинчатая правка ViewModel без UI была бы недосвязанной. Модель/клиент-слой (эта задача) даёт всю нужную инфраструктуру
- [ ] тесты ViewModel-логики черновика: сериализация items, вывод per-person сумм (клиентское превью), сброс items при ручной правке, **загрузка editOperation.items в форму (round-trip: load → save → items не потеряны)**. Частично: сериализация items и per-person превью (`Array<OperationItem>.derivedShares()` — зеркало серверного `DeriveShares`) покрыты в `ItemDraftTests.swift` на модель-слое; ViewModel-части (сброс Items при ручной правке, загрузка в форму) ⏸ ОТЛОЖЕНЫ В TASK 13 вместе с UI
