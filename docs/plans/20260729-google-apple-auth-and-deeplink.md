...
- ➕ Create: `ios/SplittyTests/AccountLinksTests.swift` — модель, запросы клиента, поведение сессии при удалении

- [x] добавить `linkedProviders` в модель `Me` (`Models.swift`)
- [x] **исправить устаревший комментарий на `Models.swift:5`** («id пользователей — Telegram user id») — после этого плана он ложь
- [x] секция «Способы входа»: список привязанных, кнопки привязать/отвязать
- [x] отвязку последнего способа блокировать **в UI** (кнопка недоступна с пояснением), не полагаясь только на 409
- [x] `409 identity_taken` показывать текстом «Этот аккаунт уже связан с другим профилем Splitty», без кода ошибки
- [x] «Удалить аккаунт» внизу экрана, деструктивным стилем, с обязательным подтверждением
- [x] в тексте подтверждения честно сказать: профиль удаляется, **а расходы и долги в группах остаются** (снимки сохраняются — иначе это ложь)
- [x] после успешного `DELETE /me` — полный `logout`, чистка Keychain, `PendingJoin` и офлайн-кеша, возврат на экран входа
- [x] написать тесты: успешное удаление приводит к разлогину и очистке; ошибка сети не разлогинивает; отвязка последнего способа заблокирована
- [x] `xcodebuild test` — зелёные перед Task 21 (197 тестов `SplittyTests`, из них 14 новых)

**Как сделано (для следующих задач):**
- `Me.linkedProviders` декодируется через `init(from:)` в extension с `decodeIfPresent ?? []` (образец — `RoomSummary.debtsUnavailable`): в офлайн-кеше `OfflineStore` лежат профили, записанные ДО появления ключа, и строгий decode ронял бы весь кешированный профиль на первом холодном старте. Поле — `var` с дефолтом, memberwise-инициализатор сохранён
- `LoginProvider` (`Models.swift`) — единственное место, где живут строки `telegram`/`google`/`apple`: тот же `rawValue` едет и в `linkedProviders`, и в путь `/me/link/{provider}`. Правило `Me.canUnlink` (привязан И способов больше одного) — чистая функция, на ней держится блокировка кнопки
- [decision] строка **Telegram показывается только когда он уже привязан**. Привязка требует Telegram Login Widget (подписанные ботом `id`/`auth_date`/`hash`), которого в приложении нет и который без веб-виджета не сделать; кнопка «Привязать» рядом с Telegram была бы обещанием несуществующего. Отвязать — можно, эндпоинт работает и предупреждение сервера показывается
- [decision] `SessionStore.init` получил `urlSession: URLSession = .shared` — тот же шов, что `APIClient` получил в Task 16. Без него требуемые планом тесты «удаление разлогинивает» и «ошибка сети не разлогинивает» не пишутся вовсе: клиента сессия создаёт сама, снаружи его подменить нечем. Прод-путь (`.shared`) не изменился
- [decision] `logout()` вызывается ТОЛЬКО после успешного `DELETE /me`. Локальные данные (Keychain, кеш, outbox, `PendingJoin`) чистит существующий `logout` — второй копии чистки не заводим, а при сетевой ошибке человек остаётся в живом аккаунте, а не на экране входа с ложным ощущением, что удаление прошло
- [decision] `PushManager.unregisterCurrentToken()` зовётся ПЕРЕД удалением (как и перед выходом): после tombstone запрос отвергнет middleware, и токен устройства остался бы висеть до первой неудачной доставки
- [decision] `identityErrorText` (`AccountView.swift`) — отдельный словарь ошибок для привязки, а не расширение `humanErrorText`: коды `identity_taken`/`last_identity` осмысленны только на этом экране, и текст обязан давать следующий шаг («войдите через него», «сначала привяжите другой»). 401 здесь тоже не «сессия протухла», а отказ провайдера
- [decision] кнопка привязки Apple — системная `SignInWithAppleButton(.continue)` шириной 170: своя вёрстка логотипа Apple в этой роли — прямой повод для отказа на ревью (та же причина, что в Task 15). Отвязка у всех провайдеров — нейтральный `.softChip`, красный остаётся только за удалением аккаунта
- ⚠️ проверить вживую привязку нельзя по тем же причинам, что в Tasks 15-16 (нет capability Sign in with Apple и `.p8`, Google-проект в статусе Testing), а удаление аккаунта — потому что оно необратимо для реального аккаунта на дев-бэкенде. Проверены сборка и 197 юнит-тестов
- 4 падающих UI-теста (`DashboardShotsUITests`, `DemoFlowUITests`, `OfflineSmokeUITests`) — предсуществующие, зависят от локального бэкенда (см. Task 15)
...
- [x] описать в `docs/API.md` новые эндпоинты: `/auth/google`, `/auth/apple`, `/me/link/{provider}`, `DELETE /me`, публичные `/join/{roomId}` и `.well-known/*`
- [x] задокументировать новые переменные окружения рядом с существующими `API_JWT_SECRET`, `API_DEV_AUTH`
- [x] описать в `README.md` модель личности: `_id` — номер Splitty, `telegram_id` — привязка; явно предупредить, что `_id ≠ telegram id` у новых пользователей
- [x] зафиксировать, что слияние аккаунтов не поддерживается и почему (денормализованные снимки)
- [x] **отметить смену семантики существующих настроек**: `REVIEW_USER_ID` (`config.go:28`) и `DAILY_EXPENSES_USERS` (`config.go:18`, дефолт из 6 telegram id) теперь означают номера Splitty — для существующих пользователей значения совпадают, но новых туда вписывать нужно по `_id`
- [x] описать в `README.md` запуск тестов репозитория (`MONGO_TEST_URI`, `docker compose up -d mongo`)
- [x] дополнить `ios/asc/INSTRUCTION.md`: capability Sign in with Apple, требования 4.8 и 5.1.1(v), **отзыв токенов через `auth/revoke` при удалении аккаунта и то, что для него нужен `.p8`-ключ Apple в `APPLE_PRIVATE_KEY`**
- [x] **предупредить в `README.md`, что `.p8`-ключ Apple и его содержимое в git не коммитятся** — только в `.env` на сервере, как и остальные секреты
- [x] перенести план в `docs/plans/completed/` — *(выполняется оркестратором после ревью)*

**Как сделано:**
- всё написанное сверено с кодом, а не с планом. Каждое имя переменной, разделитель, дефолт и код ошибки взяты из `cmd/splitty/config.go`, `internal/rest/server.go` (регистрация маршрутов), `auth.go`, `identity.go`, `delete_account.go`, `deeplink.go`, `internal/repository/{sequence,migrate}.go`
- `docs/API.md`: переписано правило «ID пользователя» (был «Telegram user id» — прямая ложь после Task 6); в список неавторизованных путей добавлены `/join/{roomId}` и `/.well-known/*`; `Me` в DTO дополнен `linkedProviders`; новые разделы «Способы входа (link/unlink)» и «Диплинк-вход в группу»; `DELETE /me` описан порядком шагов, потому что порядок здесь — контракт, а не деталь
- отдельной таблицей задокументированы одиннадцать новых переменных. Явно выделено то, на чём легко обжечься: **`ANDROID_CERT_SHA256` разделяется запятой**, а не двоеточием (сам отпечаток состоит из байтов через `:`); `APPLE_PRIVATE_KEY` — содержимое `.p8`, а не путь; Android-клиенты **не** кладутся в `GOOGLE_CLIENT_IDS`
- [decision] смену семантики `REVIEW_USER_ID`/`DAILY_EXPENSES_USERS` записал в оба документа (README — как ⚠️-подраздел модели личности, API.md — под таблицей env), а не в один: их читают разные люди — деплойщик смотрит README, интегратор клиента смотрит API.md. Перед этим проверено, что обе настройки действительно ходят в `FindById` (`internal/dailyexpenses/integration.go:61`, `internal/rest/auth.go:369`), то есть означают именно `_id`
- [decision] чекбокс про тесты репозитория закрыт существующим разделом README «Тесты репозитория» — он уже описывал и `MONGO_TEST_URI`, и `docker compose up -d mongo`. Вместо дубля дополнен перечень того, что эти тесты проверяют (sparse-индексы личностей, атомарность аллокатора, идемпотентность бэкфилла) — иначе раздел выглядел бы написанным до этого плана и не относящимся к нему
- [deviation] попутно исправлен дефолт `GEMINI_MODEL` в README и API.md: там стояло `gemini-2.0-flash`, в `config.go` — `gemini-3.1-flash-lite`. К этому плану отношения не имеет, но правило «код — истина» касается всего файла, а не только новых строк
- `ios/asc/INSTRUCTION.md`: новый раздел «Sign in with Apple» — что включить в консолях (capability на App ID, ключ в Keys, `.p8` отдаётся **один раз**), зачем это нужно по 4.8 и 5.1.1(v), и как связаны отзыв токенов и `.p8`. Явно записана механика: `authorizationCode` при входе → `auth/token` → refresh token в профиле → `auth/revoke` при `DELETE /me` **до** tombstone. Отдельно — что при пустом `APPLE_PRIVATE_KEY` вход и удаление продолжают работать, но отзыва не будет, и ревьюер увидит приложение в «Настройки → Apple ID → Вход с Apple»
- расположение кнопки удаления в INSTRUCTION.md не выдумано: `AccountView.swift` проверен — это последняя карточка вкладки «Профиль» (`deleteAccountSection`), «Способы входа» — карточка выше (`loginMethodsSection`), отдельного экрана настроек нет
- запрет на коммит `.p8` записан дважды намеренно: в README (рядом с переменными, где ключ появляется первый раз) и в разделе «⚠️ Безопасность» INSTRUCTION.md (рядом с `keys/`/`certs/`, где про такие ключи уже говорится). Секрет утекает из того файла, который человек читает, а читают эти два разных
- последний чекбокс (перенос плана в `docs/plans/completed/`) отмечен, но **файл не перемещён**: перенос делает оркестратор после фаз ревью — плану ещё предстоит принять правки
- `GOTOOLCHAIN=local ~/sdk/go1.23.5/bin/go build ./...` — зелёный (изменения чисто документационные, но проверено)
