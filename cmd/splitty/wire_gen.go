...
func initApp(ctx context.Context, cfg *config) (*events.TelegramListener, func(), error) {
	...
	joinRoom := bot.NewJoinRoom(chatStateService, buttonService, roomService, userService, botConfig)
	operationService := service.NewOperationService(mongoRoomRepository)
	statisticService := service.NewStatisticService(roomService, operationService)
	allRoomInline := bot.NewAllRoomInline(chatStateService, buttonService, roomService, statisticService, userService, botConfig)
	wantDonorOperation := bot.NewWantDonorOperation(chatStateService, buttonService, operationService, roomService, botConfig)
	roomStateService := service.NewRoomStateService(operationService, mongoRoomRepository)
	addDonorOperation := bot.NewAddDonorOperation(chatStateService, buttonService, operationService, roomService, roomStateService, botConfig)
	editDonorOperation := bot.NewEditDonorOperation(buttonService, chatStateService, operationService, roomService, botConfig)
	deleteDonorOperation := bot.NewDeleteDonorOperation(chatStateService, buttonService, operationService, roomService, userService, botConfig)
	viewRoom := bot.NewViewRoom(buttonService, roomService, chatStateService, userService, botConfig)
	viewAllOperations := bot.NewViewAllOperations(chatStateService, roomService, buttonService, operationService, botConfig)
	allRoom := bot.NewAllRoom(chatStateService, buttonService, roomService, botConfig)
	chooseRecepientOperation := bot.NewChooseRecepientOperation(chatStateService, buttonService, userService, operationService, roomService, botConfig)
	wantReturnDebt := bot.NewWantReturnDebt(chatStateService, userService, buttonService, operationService, roomService, botConfig)
	addRecepientOperation := bot.NewAddRecepientOperation(chatStateService, buttonService, operationService, userService, roomService, roomStateService, botConfig)
	viewUserDebts := bot.NewViewUserDebts(chatStateService, roomService, buttonService, operationService, botConfig)
	viewAllDebts := bot.NewViewAllDebts(chatStateService, roomService, buttonService, operationService, botConfig)
	roomSetting := bot.NewRoomSetting(buttonService, roomService, chatStateService, botConfig)
	archiveRoom := bot.NewArchiveRoom(buttonService, roomStateService, roomService, chatStateService, botConfig, roomSetting)
	...
	viewAllDebtOperations := bot.NewViewAllDebtOperations(chatStateService, roomService, buttonService, operationService, userService, botConfig)
	viewMyOperations := bot.NewViewMyOperations(chatStateService, roomService, buttonService, operationService, botConfig)
	...
	viewDonorOperation := bot.NewViewDonorOperation(buttonService, operationService, roomService, userService, botConfig)
	mongoInviteRepository := repository.NewInviteRepository(database)
	selectedLeaveRoom := bot.NewSelectedLeaveRoom(buttonService, userService, roomService, mongoInviteRepository, chatStateService, botConfig)
	viewOperationsWithMe := bot.NewViewOperationsWithMe(chatStateService, roomService, buttonService, operationService, botConfig)
	...
	editDonorAmountHandler := bot.NewEditDonorAmountHandler(buttonService, operationService, chatStateService, roomService, botConfig)
	addSplitTypeDonorOperation := bot.NewSplitTypeDonorOperation(chatStateService, buttonService, operationService, roomService, roomStateService, botConfig)
	addedDonorAmountOperation := bot.NewAddedDonorAmountOperation(chatStateService, buttonService, operationService, roomService, roomStateService, botConfig)
	chooseCurrencyRoom := bot.NewChooseCurrencyRoom(buttonService, roomStateService, roomService, chatStateService, botConfig, roomSetting)
	...
	mongoLoginCodeRepository := repository.NewLoginCodeRepository(database)
	loginCodeService := service.NewLoginCodeService(mongoLoginCodeRepository)
	loginScreen := bot.NewLoginScreen(chatStateService, loginCodeService, botConfig)
	mongoBugReportRepository := repository.NewBugReportRepository(database)
	bugReportService := service.NewBugReportService(mongoBugReportRepository)
	reportScreen := bot.NewReportScreen(chatStateService, bugReportService, userService, botConfig)
	v := ProvideBotList(operation, startScreen, roomCreating, roomSetName, joinRoom, allRoomInline, wantDonorOperation, addDonorOperation, editDonorOperation, deleteDonorOperation, viewRoom, viewAllOperations, allRoom, chooseRecepientOperation, wantReturnDebt, addRecepientOperation, viewUserDebts, viewAllDebts, roomSetting, archiveRoom, archivedRooms, statistic, viewAllDebtOperations, viewMyOperations, debt, userSetting, chooseLanguage, operationAdded, chooseNotification, selectedNotification, debtReturned, wantAddFileToOperation, addFileToOperation, viewFileOperation, viewDonorOperation, selectedLeaveRoom, viewOperationsWithMe, chooseCountInPage, finishedAddOperation, viewBankDetails, setBankDetails, wantSetBankDetails, editDonorAmountHandler, addSplitTypeDonorOperation, addedDonorAmountOperation, chooseCurrencyRoom, changePayerHandler, changedPayerHandler, unsupportedScreen, disableEnableAllDonorHandler, loginScreen, reportScreen)
	...
	telegramListener, err := initTelegramConfig(botAPI, v, buttonService, userService, chatStateService, integrationService)
	if err != nil {
		cleanup()
		return nil, nil, err
	}
...
var bots = wire.NewSet(bot.NewStartScreen, bot.NewRoomCreating, bot.NewRoomSetName, bot.NewJoinRoom, bot.NewAllRoomInline, bot.NewWantDonorOperation, bot.NewAddDonorOperation, bot.NewEditDonorOperation, bot.NewEditDonorAmountHandler, bot.NewDeleteDonorOperation, bot.NewViewRoom, bot.NewViewAllOperations, bot.NewAllRoom, bot.NewChooseRecepientOperation, bot.NewWantReturnDebt, bot.NewAddRecepientOperation, bot.NewViewUserDebts, bot.NewViewAllDebts, bot.NewRoomSetting, bot.NewArchiveRoom, bot.NewArchivedRooms, bot.NewStatistic, bot.NewViewAllDebtOperations, bot.NewOperation, bot.NewViewMyOperations, bot.NewDebt, bot.NewUserSetting, bot.NewChooseLanguage, bot.NewOperationAdded, bot.NewChooseNotification, bot.NewSelectedNotification, bot.NewDebtReturned, bot.NewWantAddFileToOperation, bot.NewAddFileToOperation, bot.NewViewFileOperation, bot.NewViewDonorOperation, bot.NewSelectedLeaveRoom, bot.NewViewOperationsWithMe, bot.NewChooseCountInPage, bot.NewFinishedAddOperation, bot.NewWantSetBankDetails, bot.NewSetBankDetails, bot.NewViewBankDetails, bot.NewSplitTypeDonorOperation, bot.NewAddedDonorAmountOperation, bot.NewChooseCurrencyRoom, bot.NewChangePayerHandler, bot.NewChangedPayerHandler, bot.NewUnsupportedScreen, bot.NewDisableEnableAllDonorHandler, bot.NewLoginScreen, bot.NewReportScreen)
