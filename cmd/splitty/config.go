...
type config struct {
	...
	// FractionalEnabledAt — момент подъёма рубильника (RFC3339). Тусы, заведённые
	// раньше, умолчание валюты не подхватывают: их настройку проставляют явно.
	// Пусто — умолчание действует для всех, как было до появления барьера.
	FractionalEnabledAt string `env:"FRACTIONAL_ENABLED_AT"`
