...
Ревьюеру НЕ нужен ни Telegram, ни Google/Apple-аккаунт — у него есть постоянный код.
Текст заметки (Sign-in required: Yes, поля Username/Password оставить пустыми, всё в Notes):

> Вход не требует стороннего аккаунта. На экране входа раскройте секцию
> **«Вход по коду из бота»** внизу, вставьте код `<REVIEW_LOGIN_CODE>` в поле
> «Код входа» и нажмите «Войти по коду». Открывать Telegram-бота не нужно —
> код постоянный и логинит в демо-аккаунт с готовыми группами и расходами.

Проверить перед отправкой (иначе ревью встанет на первом же шаге):

```bash
curl -s -o /dev/null -w '%{http_code}\n' -X POST https://splitor.zagirnur.dev/api/v1/auth/code \
  -H 'Content-Type: application/json' -d '{"code":"<REVIEW_LOGIN_CODE В ВЕРХНЕМ РЕГИСТРЕ>"}'
```

Ждём `200`. ⚠️ Сравнение на сервере идёт с `strings.ToUpper(REVIEW_LOGIN_CODE)`, а приложение
приводит ввод к верхнему регистру само (`LoginCode.normalize`) — поэтому из UI код работает в
любом регистре, а вот при проверке через curl регистр надо повысить руками. `401` на строчном
коде и `200` на том же в верхнем — это норма, а не поломка.

Демо-аккаунт (`REVIEW_USER_ID`) удалить нельзя — сервер отвечает `403`, чтобы ревьюер не убил
демо-вход следующему ревью. Google и Apple тоже доступны, но для проверки не обязательны.
