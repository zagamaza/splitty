...
- В заметках для ревьюеров обновить описание входа: демо-вход по `REVIEW_LOGIN_CODE` работает,
  плюс появились Google и Apple. Демо-аккаунт (`REVIEW_USER_ID`) удалить нельзя — сервер отвечает
  `403`, чтобы ревьюер не убил демо-вход следующему ревью.
