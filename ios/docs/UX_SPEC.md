...
Сторонние зависимости — только те, без которых интеграция невозможна в принципе, и каждая
заводится через SPM в `ios/project.yml`: `FirebaseMessaging` (пуши APNs) и `GoogleSignIn`
(системный лист входа Google — id-токен иначе не получить; подпись всё равно проверяет наш
бэкенд). Всё остальное — стандартный SDK; UI-обёртки чужих SDK не берём (`GoogleSignInSwift`
не подключён, кнопка своя). Все строки UI — по-русски, без Localizable (MVP).
