    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Button("Выйти", role: .destructive) {
                // Событие — здесь, а не в `SessionStore.logout()`: туда приходят
                // ещё удаление аккаунта и отзыв сессий, а `logout` по контракту
                // означает ровно нажатие этой кнопки. Отправка идёт мимо
                // очереди и до чистки — очередь переживёт выход не дольше
                // самого выхода.
