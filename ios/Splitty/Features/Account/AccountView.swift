...
///
/// Случаи принципиально разные.
///
/// 403 — демонстрационный аккаунт ревьюеров: аккаунт жив, сессия цела, и
/// говорить надо именно это.
///
/// Запрос НЕ ДОШЁЛ до сервера (нет сети, таймаут, битый адрес) — аккаунт тоже
/// заведомо жив, и локальные данные `SessionStore.deleteAccount` в этом случае
/// НЕ трогает. Обещать здесь «данные с устройства удалены» нельзя: это прямая
/// ложь, а человек как раз остаётся в живой сессии со своей очередью расходов.
///
/// Всё остальное (сервер ответил ошибкой) неоднозначно: tombstone он ставит
/// раньше, чем доделывает остальные шаги, так что аккаунт мог быть удалён, а
/// мог и нет — локальные данные в этом случае с устройства уже стёрты, и
/// человеку нужно сказать, что делать дальше, а не «Ошибка сервера (500)».
func deleteAccountErrorText(_ error: Error) -> String {
    ...
    if (error as? APIError)?.isServerResponse != true {
        return humanErrorText(error)
    }
