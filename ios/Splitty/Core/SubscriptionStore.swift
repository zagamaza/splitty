...
final class SubscriptionStore {
    ...
    ///
    /// Сообщение и возвращаемое значение отвечают на РАЗНЫЕ вопросы, и слить их
    /// в одно нельзя:
    ///
    ///   - сообщение — про покупки: их не нашлось, и человек должен это узнать,
    ///     каким бы ни был его тариф. Раньше оно молчало при `tier == .plus`, и
    ///     человек с подаренным Plus (или comp-аккаунт ревьюера) тапал
    ///     «Восстановить покупки» и получал молчаливый успех — восстановилось
    ///     будто бы что-то, чего никогда не покупали;
    ///
    ///   - возвращаемое значение — сигнал «закрыть пейвол» (`PaywallView`).
    ///     Оно остаётся `tier == .plus`: иначе тот же человек с грантом остался
    ///     бы стоять на экране с кнопками покупки, которые ему не нужны.
    ///
    /// Android так и делает: `return restored && isPlus`.
