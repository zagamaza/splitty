...
final class SessionStore {
    ...
    init(urlSession: URLSession = .shared) {
        ...
        // Монитор сети — только под сессией: он трогает локальные интерфейсы, и
        // система спрашивает про домашнюю сеть. Гостю этот вопрос задавать не
        // за что, у него нет ни кеша, ни очереди расходов (см. NetworkMonitor).
        if savedToken != nil { network.start() }
    }
    ...
    private func adoptSession(_ response: AuthResponse) {
        ...
        // Теперь офлайн-баннер и офлайн-ветки имеют смысл — см. init.
        network.start()
        me = response.user
    ...
    func deleteAccount() async throws {
        // Событие об удалении здесь не шлётся и послать его нельзя: чистка
        // сносит `product_events` этого человека целиком, и запись либо
        // сотрётся тем же запросом, либо переживёт удалённый аккаунт.
        // См. `docs/analytics-events.md`.
        do {
    ...
    @MainActor
    func logout() {
        // Событие выхода шлёт НЕ этот метод, а кнопка «Выйти» (`AccountView`):
        // сюда приходят и удаление аккаунта, и отзыв сессий, а `logout` по
        // контракту означает ровно нажатие кнопки. См. `docs/analytics-events.md`.
