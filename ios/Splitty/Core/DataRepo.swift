...
final class DataRepo {
    ...
    // MARK: Запись кеша без запроса

    /// Кладёт профиль в кеш «как если бы его вернул GET /me». Нужен входу:
    /// ответ /auth/* несёт тот же профиль, но мимо `me()`, и без этой записи
    /// офлайн-старт сразу после входа не находит профиль — экран группы
    /// показывает «Профиль не загружен» вместо кешированных операций.
    func cacheMe(_ me: Me) async {
        await cache.write(me, key: "\(scope)-me")
    }
