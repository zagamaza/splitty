...
UI-тестам нужен локальный бэкенд на `127.0.0.1:7171` с `API_DEV_AUTH=true` и данными,
которые они ищут по имени (группа «Поездка в Стамбул» с расходом «Ужин в ресторане»).
Каждый класс логинится сам (dev-вход под id 100) — порядок прогона не важен.
